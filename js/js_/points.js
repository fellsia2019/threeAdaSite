let NodesJs = (function (parameters) {
  let t_NodesJs = this;
  ...
  const canvas = document.getElementById(t_NodesJs.id);
  let ctx;
  let cw = document.querySelector(t_NodesJs.container).offsetWidth;
  let ch = document.querySelector(t_NodesJs.container).offsetHeight;
  ...
  let step = function () {
    ...
    t_NodesJs.nodes.forEach(function (_node, _node_i) {
        ...
        t_NodesJs.nodes.forEach(function (_node2, _node2_i) {
            ...
            let dx = Math.abs(_node[0] - _node2[0]);
            let dy = Math.abs(_node[1] - _node2[1]);
